...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            ...
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark"><b>菜單資訊</b></h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ env('APP_NAME') }}</a></li>
                        <li class="breadcrumb-item active"><a href="{{ url('foodmenus') }}">菜單資訊</a></li>
                    ...
                    <div class="card">
                            ...
                            <div class="row">
                                <div class="col-6">
                                    <div class="float-left d-flex align-items-center">
                                        ...
                                        <a href="{{ route('admin.foodmenus.create') }}" class="btn btn-sm btn-primary mr-1"><i class="fas fa-plus mr-1"></i>新增</a>
                                        @endif
                                    </div>
                                </div>
                                <div class="col-6">
                                    <div class="float-right">
                                        <div class="input-group input-group-sm align-middle align-items-middle">
                                            <span class="badge badge-purple text-lg mr-2">總筆數：{{ !empty($foodmenus) ? number_format($foodmenus->total()) : 0 }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-body">
                            <div id="search" class="card card-primary" style="display: {{ isset($searchForm) ? 'block' : 'none' }}">
                                ...
                                <form id="searchForm" role="form" action="{{ url('foodmenus') }}" method="get">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="text-left align-middle">
                                            ...
                                            <span class=""><a href="{{ route('admin.foodmenus.show', $foodmenu->id ) }}">{{ $foodmenu->title }}</a></span>
                                            @else
                                            <span class="">{{ $foodmenu->title }}</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-left align-middle">{{ $foodmenu->end_time }}</td>
                                        <td class="text-center align-middle">
                                            @if(Auth::user()->role == 'develop' || in_array($menuCode.'O',explode(',',Auth::user()->permissions)))
                                            <form action="{{ url('foodmenus/active/' . $foodmenu->id) }}" method="POST">
                                                @csrf
                                                <input type="checkbox" name="is_on" value="{{ $foodmenu->is_on == 1 ? 0 : 1 }}" data-bootstrap-switch data-on-text="啟用" data-off-text="停用" data-off-color="secondary" data-on-color="success" {{ isset($foodmenu) ? $foodmenu->is_on == 1 ? 'checked' : '' : '' }}>
                                            </form>
                                            @else
                                            <input type="checkbox" name="is_on" value="{{ $foodmenu->is_on == 1 ? 0 : 1 }}" data-bootstrap-switch data-on-text="啟用" data-off-text="停用" data-off-color="secondary" data-on-color="success" {{ isset($foodmenu) ? $foodmenu->is_on == 1 ? 'checked' : '' : '' }} disabled>
                                            @endif
                                        </td>
                                        <td class="text-center align-middle">
                                            @if(Auth::user()->role == 'develop' || in_array($menuCode.'O',explode(',',Auth::user()->permissions)))
                                            <form action="{{ url('foodmenus/preview/' . $foodmenu->id) }}" method="POST">
                                                @csrf
                                                <input type="checkbox" name="is_preview" value="{{ $foodmenu->is_preview == 1 ? 0 : 1 }}" data-bootstrap-switch data-on-text="啟用" data-off-text="停用" data-off-color="secondary" data-on-color="success" {{ isset($foodmenu) ? $foodmenu->is_preview == 1 ? 'checked' : '' : '' }}>
                                            </form>
                                            @else
                                            <input type="checkbox" name="is_preview" value="{{ $foodmenu->is_preview == 1 ? 0 : 1 }}" data-bootstrap-switch data-on-text="啟用" data-off-text="停用" data-off-color="secondary" data-on-color="success" {{ isset($foodmenu) ? $foodmenu->is_preview == 1 ? 'checked' : '' : '' }} disabled>
                                            @endif
                                        </td>
                                        <td class="text-center align-middle">
                                            @if(Auth::user()->role == 'develop' || in_array($menuCode.'S',explode(',',Auth::user()->permissions)))
                                            @if($loop->iteration != 1)
                                            <a href="{{ url('foodmenus/sortup/' . $foodmenu->id) }}" class="text-navy">
                                                <i class="fas fa-arrow-alt-circle-up text-lg"></i>
                                            </a>
                                            @endif
                                            @if($loop->iteration != count($foodmenus))
                                            <a href="{{ url('foodmenus/sortdown/' . $foodmenu->id) }}" class="text-navy">
                                        ...
                                        <td class="text-center align-middle">
                                            ...
                                            <form action="{{ route('admin.foodmenus.destroy', $foodmenu->id) }}" method="POST">
                                                @csrf
                        ...
                        <div class="card-footer bg-white">
                            <div class="float-left">
                                <span class="badge badge-primary text-lg ml-1">總筆數：{{ !empty($foodmenus) ? number_format($foodmenus->total()) : 0 }}</span>
                            </div>
                            <div class="float-right">
                                @if(isset($appends))
                                {{ $foodmenus->appends($appends)->render() }}
                                @else
                                {{ $foodmenus->render() }}
                                @endif
                            </div>
