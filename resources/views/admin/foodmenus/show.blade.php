...
<div class="content-wrapper">
    <div class="content-header">
        ...
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark"><b>菜單資訊</b><small> ({{ isset($foodmenu) ? '修改' : '新增' }})</small></h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ env('APP_NAME') }}</a></li>
                        <li class="breadcrumb-item"><a href="{{ url('foodmenus') }}">菜單資訊</a></li>
                        <li class="breadcrumb-item active">{{ isset($foodmenu) ? '修改' : '新增' }}菜單</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <section class="content">
        <div class="container-fluid">
            @if(isset($foodmenu))
            <form id="delImgForm" action="{{ route('admin.foodmenus.delimg', $foodmenu->id) }}" method="POST">
                @csrf
            </form>
            @endif
            <form id="myform"
                  action="{{ isset($foodmenu) ? route('admin.foodmenus.update', $foodmenu->id) : route('admin.foodmenus.store') }}"
                  method="POST"
                  enctype="multipart/form-data"
                  data-parsley-validate>
                @csrf
                @if(isset($foodmenu)) @method('PATCH') @endif
                <input type="hidden" name="type" value="foodmenu">

                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">{{ $foodmenu->title ?? '新增' }}</h3>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-md-4">
                                {{-- <label for="title"><span class="text-red">*</span> {{ __('validation.attributes.foodmenu.title') }}</label> --}}
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <input type="text"
                                       ...
                                       value="{{ old('description', $foodmenu->description ?? '') }}"
                                       placeholder="請輸入描述"
                                ...
                                <div class="row">
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type="text"
                                               ...
                                               value="{{ old('start_time', $foodmenu->start_time ?? '') }}"
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type="text"
                                               ...
                                               value="{{ old('end_time', $foodmenu->end_time ?? '') }}"
                                    ...
                                    <div class="form-group col-md-12">
                                        <label for="img">菜單圖片 (@if($imageSetting->width > 0)上傳圖片寬度超過 {{ $imageSetting->width }}px，將會等比例自動調整@endif)</label>
                                        <div class="input-group">
                                            <div class="custom-file">
                                                ...
                                                <label class="custom-file-label" for="img" id="img-label">
                                                    {{ old('img', isset($foodmenu) && $foodmenu->img ? basename($foodmenu->img) : __('選擇圖片')) }}
                                                </label>
                                            </div>
                                            @if(isset($foodmenu) && in_array($menuCode.'M',explode(',',Auth::user()->permissions)))
                                        ...
                                        {{-- 已存在圖片（編輯時） --}}
                                        @if(isset($foodmenu) && $foodmenu->img)
                                        <div class="mt-2">
                                            <img src="{{ asset('storage/upload/' . $foodmenu->img) }}" width="400" alt="{{ $foodmenu->title }}" class="border rounded old-image">
                                        </div>
                                        @endif
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <textarea class="form-control @error('content') is-invalid @enderror"
                                          ...
                                          data-parsley-trigger="change">{{ old('content', $foodmenu->content ?? '') }}</textarea>
                    ...
                    <div class="card-footer text-center bg-white">
                        @if((isset($foodmenu) && in_array($menuCode.'M',explode(',',Auth::user()->permissions))) || in_array($menuCode.'N',explode(',',Auth::user()->permissions)))
                        <button id="modifyBtn" type="button" class="btn btn-primary">{{ isset($foodmenu) ? '修改' : '新增' }}</button>
                        @endif
                        <a href="{{ url('foodmenus') }}" class="btn btn-info">
...
<script>
    // 保存原始圖片預覽與檔名
    const originalImgSrc = '{{ isset($foodmenu) && $foodmenu->img ? asset("storage/upload/" . $foodmenu->img) : "" }}';
    const originalImgName = '{{ isset($foodmenu) && $foodmenu->img ? basename($foodmenu->img) : __("選擇圖片") }}';
    ...
    window.Parsley.addValidator('filemaxmegabytes', {
        ...
        messages: {
            zh_tw: "{{ __('validation.attributes.foodmenu.messages.img.filemaxmegabytes', ['max' => 5]) }}",
            zh_cn: "{{ __('validation.attributes.foodmenu.messages.img.filemaxmegabytes', ['max' => 5]) }}",
            en: "{{ __('validation.attributes.foodmenu.messages.img.filemaxmegabytes', ['max' => 5]) }}",
        }
    });
    ...
    window.Parsley.addValidator('filemimetypes', {
        ...
        messages: {
            zh_tw: "{{ __('validation.attributes.foodmenu.messages.img.filemimetypes', ['types' => 'jpeg, png, jpg, gif, svg']) }}",
            zh_cn: "{{ __('validation.attributes.foodmenu.messages.img.filemimetypes', ['types' => 'jpeg, png, jpg, gif, svg']) }}",
            en: "{{ __('validation.attributes.foodmenu.messages.img.filemimetypes', ['types' => 'jpeg, png, jpg, gif, svg']) }}",
        }
    });
