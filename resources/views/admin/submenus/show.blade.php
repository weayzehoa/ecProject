                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control {{ $errors->has('func_code') ? ' is-invalid' : '' }}" id="func_code" name="func_code" value="{{ $subMenu->func_code ?? '' }}" placeholder="請輸入功能代碼">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control {{ $errors->has('icon') ? ' is-invalid' : '' }}" id="icon" name="icon" value="{{ $subMenu->icon ?? ''}}" placeholder="請輸入Font Awesome 5圖示完整語法，可在class中加上其他語法">
                                            @if ($errors->has('icon'))
                                            <span class="invalid-feedback" role="alert">
                                        ...
                                        <div id="url_d" class="form-group" {{ isset($subMenu) ? $subMenu->url_type == 0 ? 'd-none' : '' : 'd-none'}}>
                                        ...
                                        <div id="power_d" class="form-group" {{ isset($subMenu) ? $subMenu->url_type == 0 ? 'd-none' : '' : 'd-none'}}>
                                            <label for="url">提供的功能</label>
                                            <div class="row col-md-12">
                                            @foreach($poweractions as $poweraction)
                                            @if(isset($subMenu) && in_array($poweraction->code, explode(',',$subMenu->power_action)))
                                            <div class="icheck-primary mr-2">
                                                <input type="checkbox" id="pachkbox{{ $loop->iteration }}" name="power_action[]" value="{{ $poweraction->code }}" {{ isset($subMenu) ? in_array($poweraction->code, explode(',',$subMenu->power_action)) ? 'checked' : '' : '' }} disabled>
                                                <label for="pachkbox{{ $loop->iteration }}">{{ $poweraction->name }}</label>
                                            </div>
                                            @else
                                            <div class="icheck-primary mr-2">
                                                <input type="checkbox" id="pachkbox{{ $loop->iteration }}" name="power_action[]" value="{{ $poweraction->code }}">
                                                <label for="pachkbox{{ $loop->iteration }}">{{ $poweraction->name }}</label>
                                            </div>
                                            @endif
                                            @endforeach
                                            </div>
                                            @if ($errors->has('power_action') || $errors->has('power_action.*'))
                                                <div class="text-danger">
                                                    @foreach ($errors->get('power_action') as $msg)
                                                        <div>{{ $msg }}</div>
                                                    @endforeach
                                                    @foreach ($errors->get('power_action.*') as $msgs)
                                                        @foreach ($msgs as $msg)
                                                            <div>{{ $msg }}</div>
                                                        @endforeach
                                                    @endforeach
