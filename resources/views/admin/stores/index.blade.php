...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            ...
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark"><b>分店資訊</b></h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ env('APP_NAME') }}</a></li>
                        <li class="breadcrumb-item active"><a href="{{ url('stores') }}">分店資訊</a></li>
                    ...
                    <div class="card">
                            ...
                            <div class="row">
                                <div class="col-6">
                                    <div class="float-left d-flex align-items-center">
                                        ...
                                        <a href="{{ route('admin.stores.create') }}" class="btn btn-sm btn-primary mr-1"><i class="fas fa-plus mr-1"></i>新增</a>
                                        @endif
                                    </div>
                                </div>
                                <div class="col-6">
                                    <div class="float-right">
                                        <div class="input-group input-group-sm align-middle align-items-middle">
                                            <span class="badge badge-purple text-lg mr-2">總筆數：{{ !empty($stores) ? number_format($stores->total()) : 0 }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-body">
                            <div id="search" class="card card-primary" style="display: {{ isset($searchForm) ? 'block' : 'none' }}">
                                ...
                                <form id="searchForm" role="form" action="{{ url('stores') }}" method="get">
                                    <input type="hidden" name="searchForm" value="1">
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-md-3">
                                                <label for="keyword">關鍵字搜尋: (店名、電話、地址)</label>
                            ...
                            <table class="table table-hover table-sm">
                                <thead>
                                    <tr>
                                        <th class="text-left" width="10%">分店店名</th>
                                        <th class="text-left" width="10%">電話</th>
                                        <th class="text-left" width="20%">地址</th>
                                        <th class="text-left" width="10%">營業時間開始</th>
                                        <th class="text-left" width="10%">營業時間結束</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-left align-middle">{{ $store->tel }}</td>
                                        <td class="text-left align-middle">{{ $store->address }}</td>
                                        ...
                                        <td class="text-center align-middle">
                                            ...
                                            @else
                                            <input type="checkbox" name="is_on" value="{{ $store->is_on == 1 ? 0 : 1 }}" data-bootstrap-switch data-on-text="啟用" data-off-text="停用" data-off-color="secondary" data-on-color="success" {{ isset($store) ? $store->is_on == 1 ? 'checked' : '' : '' }} disabled>
                                            @endif
                                        </td>
                                        <td class="text-center align-middle">
                                            @if(in_array($menuCode.'O',explode(',',Auth::user()->permissions)))
                                            ...
                                            @else
                                            <input type="checkbox" name="is_preview" value="{{ $store->is_preview == 1 ? 0 : 1 }}" data-bootstrap-switch data-on-text="啟用" data-off-text="停用" data-off-color="secondary" data-on-color="success" {{ isset($store) ? $store->is_preview == 1 ? 'checked' : '' : '' }} disabled>
                                            @endif
                                        </td>
                                        <td class="text-center align-middle">
                                            ...
                                            @endif
                                            @if($loop->iteration != count($stores))
                                            <a href="{{ url('stores/sortdown/' . $store->id) }}" class="text-navy">
                        ...
                        <div class="card-footer bg-white">
                            <div class="float-left">
                                <span class="badge badge-primary text-lg ml-1">總筆數：{{ !empty($stores) ? number_format($stores->total()) : 0 }}</span>
                            </div>
                            <div class="float-right">
                                @if(isset($appends))
                                {{ $stores->appends($appends)->render() }}
                                @else
                                {{ $stores->render() }}
                                @endif
                            </div>
